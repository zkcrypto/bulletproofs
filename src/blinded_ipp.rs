...
impl BlindedInnerProductProof {
    ...
    pub fn create(
        ...
        // Here we need to generate F' from Gentry et al from the transcript, which we could call Q_prime.
        // We can generate it as:
        // let z = transcript.challenge_scalar(b"z");
        // let Q_prime = h_prime * z, where h_prime is a new generator
        let z = transcript.challenge_scalar(b"z");
        ...
        if n != 1 {
            ...
            for i in 0..n {
                ...
                // Compute generators for the next iteration
        ...
        // In the base case the version without zero knowledge sends a,b
        // The version with zero knowledge does a schnorr style protocol
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        buf.extend_from_slice(self.a_blinded.as_bytes());
        buf.extend_from_slice(self.b_blinded.as_bytes());
        buf.extend_from_slice(self.delta_blinded.as_bytes());
        buf
    }
    ...
    pub(crate) fn to_bytes_iter(&self) -> impl Iterator<Item = u8> + '_ {
        self.L_vec
            ...
            .chain(self.a_blinded.as_bytes())
            .chain(self.b_blinded.as_bytes())
            .chain(self.delta_blinded.as_bytes())
            .copied()
    }
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<BlindedInnerProductProof, ProofError> {
        ...
        let a_blinded = Scalar::from_canonical_bytes(read32(&slice[pos + 64..]))
            .ok_or(ProofError::FormatError)?;
        let b_blinded = Scalar::from_canonical_bytes(read32(&slice[pos + 96..]))
            .ok_or(ProofError::FormatError)?;
